...
function prependChatPMsg(msg) {
    $('#history-chat-list').prepend('<li data-toggle="popover" data-cBelongToID="' + msg.cBelongToID + '" data-cId="' + msg.cId + '">' +
        '<a href="javascript:void(0);" class="list-group-item">' +
            '<img src="http://closer-chat.coding.io' + msg.uImage + '" alt="头像" width="30px" height="30px" class="pull-left">' +
...
checkIsLogin(function() {
    ...
    socket.on('roomClientNum', function(rId, num) {
        // 更新
        $('#room-list-wrap, #room-search-wrap').find('.panel-body').find('a').each(function() {
            var $that = $(this);
        ...
        $.ajax({
            type: 'post',
            url: 'http://closer-chat.coding.io/updateRoomClientNum',
            data: {
                rId: rId,
                num: num
            },
