...
function getRoomsList(pn, area) {
    ...
    // 按分类获取的url 默认获取热门房间
    var url = area 
        ? 'http://closer-chat.coding.io/getRoomsList/:pn=' + pn + '/:tag=' + area 
        : 'http://closer-chat.coding.io/getRoomsList/:pn=' + pn + '/:tag=hot';  
            ...
            for (var i = 0, j = data.length; i < j; i++) {              
                ...
                var popoverOwnerContent = '<p class="user-login-state">当前：' + (data[i].uLoginState === 'down' ? '不' : '') + '在线</p>' +
                                            '<p>' +
                                                ...
                                                '<a href="http://closer-chat.coding.io/user?id=' + data[i].rOwnerId + '" data-isFriends="{F' + isFriend + 'F}" style="word-break: break-all;">' + data[i].uNickName + '</a></p>' +
                ...
                temp.push('<div class="col-sm-3 col-xs-4">' +
                                '<div class="text-center">' +
                                    ...
                                    '<a data-rId="' + data[i].rId + '" href="http://closer-chat.coding.io/room?id=' + data[i].rId + '" class="room-logo">' +
                                        '<img src="http://closer-chat.coding.io' + data[i].rImage + '" alt="room" width="100%" height="100%">' +
                                    ...
                                    '<div class="caption room-info">' +
                                        '<p>房号: <span class="room-id" data-toggle="popover" data-content="' + popoverIDContent + '"><a href="http://closer-chat.coding.io/room?id=' + data[i].rId + '">' + data[i].rId + '</a></span></p>' +
...
$(function() {
     $('#friends-panel-wrap').load('http://closer-chat.coding.io/friends_panel', function() {
        $('#common-modal-wrap').load('http://closer-chat.coding.io/common_modal', function() {
            ...
            $.ajax({
                type: 'GET',
                url: 'http://closer-chat.coding.io/getCampus',
                success: function(data) {
            ...
            $('#c-room-logo-div').click(function() {
                ...
                function uploadPhoto(imageURI) {
                    ...
                    var temp = imageURI.substr(imageURI.lastIndexOf('/') + 1);
                    options.fileName = (temp.indexOf('.') !== -1 ? temp : temp + '.png');
                    options.mimeType = "text/plain";
                    //alert(options.fileName)
                    $('#c-room-logo-preview').find('img')[0].src = imageURI;
                    ...
                    //alert(encodeURI("http://closer-chat.coding.io/newRoom"))
                    ft.upload(imageURI, encodeURI("http://closer-chat.coding.io/newRoom"), win, fail, options);
                }
            ...
            $('#create-room-btn').click(function() {
                ...
                $('#create-room-form').attr('action','http://closer-chat.coding.io/newRoom').submit();
            });
