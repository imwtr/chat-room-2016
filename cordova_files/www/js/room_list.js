...
$(function() {
     $('#friends-panel-wrap').load('http://closer-chat.coding.io/friends_panel', function() {
        $('#common-modal-wrap').load('http://closer-chat.coding.io/common_modal', function() {
            searchMenuHandle();
            friendPanelHandle();
            floatChatbindEvent();
            ...
            function getRoomInfo() {    
                ...
                $.ajax({
                    type: 'get',
                    url: 'http://closer-chat.coding.io/getRoomInfo/:id=' + rId,
                    success: function(data) {
                        ...
                        if (data.status == 1) {
                            ...
                            $rImage.attr('src', 'http://closer-chat.coding.io' + data.rImage);
                            // 聊天区的房名以及描述
                            $rTitle.text(data.rName);
                            $rTitleDesc.text(data.rDesc).attr('title', data.rDesc);

                            setTimeout(function() {
                                $('#userList-liked').find('span').eq(0).text($('#userList-liked').find('li').length);
                                $('#userList-unliked').find('span').eq(0).text($('#userList-unliked').find('li').length);
                                $('#room-info-content').find('p').eq(8).find('span').text($('#userList-liked').find('li').length + $('#userList-unliked').find('li').length);
                            }, 0);
                            ...
                            var popoverOwnerContent = '<p class="user-login-state">当前：' + (data.rOwner.uLoginState === 'down' ? '不' : '') + '在线</p>' +
                                                        '<p>' +
                                                            ...
                                                            '<a href="http://closer-chat.coding.io/user?id=' + data.rOwner.uId + '" data-isFriends="{F' + isFriend + 'F}" style="word-break: break-all;">' + data.rOwner.uNickName + '</a></p>' +
                            ...
                            $rOwner.attr('data-uId', data.rOwner.uId).html('房主：<a href="javascript:void(0);"  class="user-info-popover" data-toggle="popover" data-content="' + popoverOwnerContent + '">' + data.rOwner.uNickName + '</a>');
                            ...
                            //                                  '<a href="http://closer-chat.coding.io/user?id=' + data.rManagers[i].uId + '" style="word-break: break-all;">' + data.rManagers[i].uNickName + '</a></p>' +
                            //                          '<p><button data-tag="private" type="button" class="btn btn-info btn-xs ' + btnDisabled + '"><span class="glyphicon glyphicon-earphone"></span> 私信给TA</button></p>' +
                            //                          '<p><button data-tag="friend" data-act="1" type="button" class="btn btn-primary btn-xs {be ' + beFriends + ' eb}"><span class="glyphicon glyphicon-plus"></span> 加为好友</button></p>' +
                            //                          '<p><button data-tag="friend" data-act="0" type="button" class="btn btn-warning btn-xs {is ' + isFriends + ' si}"><span class="glyphicon glyphicon-remove"></span> 删除好友</button></p>' +
                            //                          '<p><button data-tag="group" type="button" class="btn btn-info btn-xs ' + btnDisabled + '"><span class="glyphicon glyphicon-earphone"></span> 讨论组</button></p>';
            ...
            $likeBtn.click(function() {
                ...
                $.ajax({
                    type: 'post',
                    url: 'http://closer-chat.coding.io/likeRoom',
                    data: {
                        rId: rId,
                        act: act
                    },
            ...
            //  折叠房间信息列表
            $('#room-info').find('span').eq(0).click(function() {
                $(this).addClass('hide').next().removeClass('hide');
                $('#room-info').find('.panel-body').hide();
            });
            // 展开房间信息列表
            $('#room-info').find('span').eq(1).click(function() {
                $(this).addClass('hide').prev().removeClass('hide');
                $('#room-info').find('.panel-body').show();
            });
        });
    });
});
