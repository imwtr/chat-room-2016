	...
	$('#chat-left-menu').find('ul').delegate('li', 'click', function(e) {
		...
		if (node.getAttribute('data-act') === 'close') {
			...
			if ($span0.attr('data-type') === 'chatP') {
				...
				$('#chat-title').attr('href', '/user?id=' + objId).attr('target', '_blank');

				if ($span0.attr('data-isFriends') == '1') {
					aHtml = '<a href="#" class="text-warning" data-act="0"  data-val="' + id + '" data-id="' + objId + '" data-type="chatP" data-toggle="tooltip" data-title="删除好友">&nbsp;<span class="glyphicon glyphicon-remove"></span></a>';
				} else {
					aHtml = '<a href="#" class="text-primary" data-act="1"  data-val="' + id + '" data-id="' + objId + '" data-type="chatP" data-toggle="tooltip" data-title="TA还不是你好友哦，点击添加好友">&nbsp;<span class="glyphicon glyphicon-plus"></span></a>';
				}

			} else {
				$('#chat-title').attr('target', '_self').attr('href', '#');
				aHtml = '<a href="#" class="text-warning" data-val="' + id + '" data-id="' + objId + '" data-type="chat" data-toggle="tooltip" data-title="退出讨论组">&nbsp;<span class="glyphicon glyphicon-log-out"></span></a>';
			}
...
function floatChatHandle(type, objId, isFriends, tag, id, img, name, signature, number) {
	...
	if (tag === 'new') {
		...
		if (!$item) {
			...
			// 在聊天浮窗中添加其会话iframe 
			$('#chat-right-content').append('<iframe data-val="' + id + '" id="chat-content-frame-' + id + '" src="' + type + '?id=' + id.slice(5) + '" frameborder="0"></iframe>');
		}
