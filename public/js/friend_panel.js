				...
				success: function(data) {
					...
					if (data.status == 1) {
						...
						for (var i = 0; i < data.group.length; i++) {
							...
							liTemp.push('<li class="list-group-item" data-gId="' + data.group[i].gId + '">' +
						...
						$('#group-list').contextmenu(function() {
							...
							html: true,
							content: '<ul class="list-unstyled">' +
								'<li><a href="#" data-act="enterG">进入讨论组</a></li>' +
								'<li><a href="#" data-act="quitG" class="text-warning">退出讨论组</a></li>',
							placement: 'left'
						});
					} else if (data.status == 0) {
	...
	function friendPanelHandle() {
		...
		$('#my-friends').click(function() {
			if ($userFriendsPanel.hasClass('in')) {

				if ($('#room-list-wrap').length) {
					$('#room-list-wrap').removeClass('col-sm-6').addClass('col-sm-9');
				} else {
					$('#chat-content-wrapper').removeClass('col-sm-6').addClass('col-sm-9');
				}
				...
				if ($('#room-list-wrap').length) {
					...
					$('#chat-content-wrapper').removeClass('col-sm-9').addClass('col-sm-6');
				}
		...
		function bindMyFriendsEvent() {
			$('#friends-list').delegate('a', 'click', function() {
				var act = $(this).attr('data-act');
				var $ul = $(this).closest('ul');
				var theUID = $ul.attr('id');
				theUID = theUID.slice(theUID.lastIndexOf('-') + 1);
				var theUNickName = $ul.find('h6').text();
			...
			if ($that.attr('data-type') === 'chat') {
				...
				if ($that.attr('data-act') === '1') {
					...
					changeUserGroup(objId, function() {
						$that.prop('outerHTML', '<a href="#" class="text-warning" data-val="' + id + '" data-act="0" data-id="' + objId + '" data-type="chatP" data-toggle="tooltip" data-title="删除好友">&nbsp;<span class="glyphicon glyphicon-remove"></span></a>');
				...
				// 删除好友
					...
					deleteFriend(objId, function() {
						$that.prop('outerHTML', '<a href="#" class="text-primary" data-val="' + id + '" data-act="1" data-id="' + objId + '" data-type="chatP" data-toggle="tooltip" data-title="TA还不是你好友哦，点击添加好友">&nbsp;<span class="glyphicon glyphicon-plus"></span></a>');
