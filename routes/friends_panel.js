		...
		global.modelHandle('group').count(function(err, total) {
			if (err) {
				...
				global.modelHandle('group').create({
					...
					gUserId: gUserId,
					gCount: gCount,
					gTime: global.Util.formatDate(new Date(), 'yyyy/mm/dd hh:ii:ss')
				}, function(err, data) {
					if (err) {
						console.log(err);
					} else {
