    ...
    function searchMenuHandle() {
        ...
        function searchByInput(e) {
            ...
            window.location = 'http://closer-chat.coding.io/search?tag=' + tag[condition] + '&w=' + encodeURIComponent(inputVal);
    ...
    function getMyFriends() {
            ...
            $.ajax({
                type: 'get',
                url: 'http://closer-chat.coding.io/getMyFriends',
        ...
        function groupRenameHandle() {
            $('.friends-list-group-rename').click(function() {
                ...
                if ($(this).attr('data-val') == 0) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: 'http://closer-chat.coding.io/renameUGroup',
                        data: {
                            ugId: ugId,
        ...
        function getLastHistoryChat() {
            ...
            $.ajax({
                type: 'get',
                url: 'http://closer-chat.coding.io/getLastHistoryChat',
                success: function(data) {
                    // 已登录则可获取信息并渲染
                    if (data.status == 1) { 
                        // 先按时间顺序排好序  降序
                        ...
                        for (var i = 0; i < chats.length; i++) {
                            ...
                            liTemp.push('<li data-toggle="popover" data-cBelongToID="' + chats[i].cBelongToID + '" data-cId="' + chats[i].cId + '">' +
                                            '<a href="javascript:void(0);" class="list-group-item">' +
                                                '<img src="http://closer-chat.coding.io' + logo + '" alt="头像" width="30px" height="30px" class="pull-left">' +
        ...
        function getMyGroups() {
            $.ajax({
                type: 'get',
                url: 'http://closer-chat.coding.io/getMyGroups',
                success: function(data) {
                    // 已登录  成功获取并渲染
                    if (data.status == 1) {
                        var liTemp = [];
    ...
    function friendPanelHandle() {
        ...
        function bindHistoryChatEvent() {
            ...
            $('#history-chat-list').delegate('[data-val="remove-chat"]', 'click', function() {
                ...
                $.ajax({
                    type: 'post',
                    url: 'http://closer-chat.coding.io/removeChatFromPanel',
                    data: {
                        cId: cId
                    },
                    success: function(data) {
        ...
        function quitFromGroup(gId, cb) {
            $.ajax({
                type: 'post',
                url: 'http://closer-chat.coding.io/quitFromGroup',
                data: {
                    gId: gId
                },
                success: function(data) {
        ...
        function panelSearch(keyWord, tag, isMore, cb) {
            ...
            $.ajax({
                type: 'post',
                url: 'http://closer-chat.coding.io/panelSearch',
                data: {
                    keyWord: keyWord,
                    tag: tag,
                    isMore: isMore
