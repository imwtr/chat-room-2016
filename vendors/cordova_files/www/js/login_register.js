...
$(function() {
    $('#registerlogin-modal-wrap').load('http://closer-chat.coding.io/login_register', function(response) {
        ...
        $.ajax({
            type: 'GET',
            url: 'http://closer-chat.coding.io/checkLoginState',
            success: function(data) {
                if (data.status == 1) {
                    $('#loginState').hide();

                    // 渲染右上角用户信息栏
                    $('#userInfo').find('a').eq(0).attr('href', 'http://closer-chat.coding.io/user?id=' + data.uId);
                    $('#userInfo').find('img').eq(0).attr('src', 'http://closer-chat.coding.io' + data.uImage);
                    $('#userInfo').find('a').eq(1).html('&nbsp;' + data.uNickName + '<span class="caret"></span>');
                    $('#userInfo').find('ul').eq(0).find('a').eq(0).attr('href', 'http://closer-chat.coding.io/user?id=' + data.uId);
                    $('#userInfo').find('ul').eq(0).find('a').eq(2).attr('href', 'http://closer-chat.coding.io/user?id=' + data.uId + '#user-set');

                    $('#userInfo').show();

                    $('#user-friends-panel-menu').find('li').last().attr('data-href', 'http://closer-chat.coding.io/user?id=' + data.uId);
        ...
        function emailCodeValidate() {
            ...
            $('#email-validate-btn').text('发送至邮箱验证').show().click(function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'http://closer-chat.coding.io/postEmailCode',
                    data: {
        ...
        function checkEmailValidate() {
            $('#modal-register-user').blur(function() {
                ...
                if (!val) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: 'http://closer-chat.coding.io/checkEmailInfo',
                        data: {
                            uEmail: val
                        },
                        success: function(data) {
        ...
        $('#modal-login-div').find('button[type="submit"]').click(function() {
            ...
            if (!userVal || !pwdVal) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'http://closer-chat.coding.io/checkLoginInfo',
                    data: {
                        uEmail: userVal,
                        uPassword: pwdVal
                    },
        ...
        $('#modal-register-div').find('button[type="submit"]').click(function() {
            ...
            if (!userVal || !pwdVal || !pwdRVal || !nickNameVal) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'http://closer-chat.coding.io/checkRegisterInfo',
                    data: {
                        uEmail: userVal,
                        uPassword: pwdVal,
        ...
        $('#user-logout').click(function() {
            $.ajax({
                type: 'POST',
                url: 'http://closer-chat.coding.io/logout',
                success: function(data) {
                    // 注销成功
                    if (data.status == 1) {
                        window.location.reload();
        ...
        $('#pwdBack-btn').click(function() {
            ...
            if (!userVal) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'http://closer-chat.coding.io/pwdBack',
                    data: {
                        uEmail: userVal
                    },
                    success: function(data) {
